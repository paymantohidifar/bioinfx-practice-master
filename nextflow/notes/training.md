...
* **Channels for Parallelism:** Nextflow uses **channels** (queues) to handle multiple input data points efficiently and enable processing them in **parallel**.
* **Loading Data:** Data from an input file (e.g., `greetings.csv`) is loaded into a channel using functions like `channel.fromPath()`, followed by **operators** (e.g., `.splitCsv()` and `.map{...}`) to parse and extract the relevant data elements.
    * The workflow then calls the process (e.g., `sayHello(greeting_ch)`) once for **each element** in the channel.
...
Here's a breakdown of the three operators you asked about:
